...
* Explore and filter application logs using LokiStack and LogQL
* Explain how metrics and logs work together for agentic observability

== What makes agentic metrics different?

Traditional applications need request rate, error rate, and latency. Agentic applications need all of that *plus* metrics that capture LLM behavior, agent routing, and tool execution. Here are the six metrics that matter most:
...
A Grafana instance is pre-deployed in your workspace namespace with a **Mortgage-AI Backend Metrics** dashboard already configured. It connects to two Prometheus datasources:

* **User Workload Monitoring** -- scrapes the mortgage-ai application metrics (default)
* **RHOAI Prometheus** -- provides model serving metrics from OpenShift AI

Let's open it and see what's there.

. Find the Grafana route and open it in your browser:
+
[source,bash]
----
GRAFANA_ROUTE=$(oc get route -n wksp-{user} grafana-route -o jsonpath='{.spec.host}')
...
. Navigate to the **Mortgage-AI Backend Metrics** dashboard, or open it directly:
+
[source,bash]
----
echo "Dashboard: https://${GRAFANA_ROUTE}/d/mortgage-ai-backend"
----
...
[%collapsible]
.PromQL details for Overview panels (click to expand)
====
...
The four stat panels at the top show:
...
Now let's see the dashboard in action. Open the mortgage-ai UI and chat with the agents:

. Open the mortgage-ai UI:
+
[source,bash]
----
UI_ROUTE=$(oc get route -n mortgage-ai mortgage-ai-ui -o jsonpath='{.spec.host}')
echo "Mortgage-AI UI: https://${UI_ROUTE}"
----

. Send several chat messages to different agent personas.

. Return to the Grafana dashboard, set the time range to **Last 15 minutes**, and watch:
+
--
* `active_chat_sessions` increments when you open a chat
* `llm_tokens_total` increases with each LLM call
* `tool_calls_total` appears when agents invoke tools
* `agent_routing_total` shows whether your query was classified as simple or complex
--

TIP: The dashboard is managed as a `GrafanaDashboard` custom resource and version-controlled via GitOps. Run `oc get grafanadashboard -n wksp-{user}` to see it.
...
oc get pods -n wksp-{user} -l app=grafana

# Confirm dashboard exists
oc get grafanadashboard -n wksp-{user}
----

✓ Dashboard loads in Grafana +
✓ Overview stats show real data after generating traffic +
✓ AI Agent Metrics panels display token usage and tool call data
...
[source,logql]
----
{ log_type="application", kubernetes_namespace_name="mortgage-ai" }
----
+
...
[source,logql]
----
{ log_type="application", kubernetes_namespace_name="mortgage-ai", kubernetes_container_name="ui" } |= "/api/"
----
+
...
[source,logql]
----
{ log_type="application", kubernetes_namespace_name="mortgage-ai", kubernetes_container_name="ui" } |= "/api/chat"
----
+
...
=== Generate traffic and find it

[start=5]
. Open the mortgage-ai UI and start a chat conversation with one of the agents:
+
[source,bash]
----
UI_ROUTE=$(oc get route -n mortgage-ai mortgage-ai-ui -o jsonpath='{.spec.host}')
...
. Return to the log viewer and watch for your chat session appearing in real-time. WebSocket connections show as HTTP 101 upgrades.
...
[source,logql]
----
{ log_type="application", kubernetes_namespace_name="mortgage-ai", kubernetes_container_name="api" }
----
...
You can see the same `/api/` requests in both the UI and API logs -- the UI shows the incoming request, the API shows how it was processed.
====
...
✓ OpenShift Console Observe -> Logs loads successfully +
✓ LogQL queries filter to mortgage-ai namespace +
✓ You can find log entries corresponding to your chat traffic +
✓ You can distinguish API vs UI log formats

== Module summary

**What you accomplished:**

* Explored a pre-deployed Grafana dashboard with real-time agent metrics
* Understood how traditional HTTP metrics and AI-specific metrics work together
* Generated traffic and observed metrics update in real-time
* Investigated application logs with LokiStack and LogQL queries
* Correlated log entries with user actions across API and UI containers

**Key takeaways:**

* Agentic apps need metrics beyond HTTP: token usage, inference latency, routing decisions, and tool success rates
* Logs provide the detail that metrics aggregate away -- essential for debugging ephemeral containers and multi-agent correlation
* Grafana dashboards and LokiStack are both managed as platform resources, pre-deployed via GitOps
