...
----
+
Alternatively, you can find the Grafana route in the OpenShift Console. Navigate to **Networking > Routes** in the `{user_project}` project and click the `grafana-route` Location link:
+
...
+
Or use the **application launcher** (waffle icon) in the top-right corner and click **Grafana ({user_project})** under **Third Party Services**:
+
...
. Optionally, navigate to the **Mortgage-AI Backend Metrics** dashboard directly:
+
[source,bash]
----
