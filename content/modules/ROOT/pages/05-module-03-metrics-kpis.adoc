...
TIP: You can access the Grafana dashboard directly from the *Grafana* tab at the top of this page, without navigating through the OpenShift Console.
...
----
+
. Grafana uses OpenShift OAuth for authentication. When prompted, log in with your OpenShift credentials: username `{user}`, password `{password}`:
+
image::agentic-app-pre7.png[OpenShift SSO login page for Grafana authentication,link=self,window=blank,width=400]
