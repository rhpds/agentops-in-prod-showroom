...
. Run the **Install Dependencies** cell, then **restart the kernel** when prompted. Run the **Setup** cells to configure the environment variables (these are the same as Module 5).
...
[start=5]
. Switch to the MLflow UI. Navigate to **Prompts** and click on **public-assistant-system-prompt**. Click the **Compare** tab. MLflow shows a side-by-side diff of Version 2 (left) and Version 1 (right):
+
...
+
The traces are grouped by prompt version. You can see exactly which test cases passed or failed for each version: Version 1 traces show consistent `True` values for `contains_expected`, while Version 2 traces show `False` for several test cases. The per-trace detail lets you click into any failing trace to understand _why_ the response missed the expected keyword (the agent answered from general knowledge instead of calling the `product_info` tool).
