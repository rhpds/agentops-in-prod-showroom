...
. Open the OpenShift Console using the *OCP Console* tab at the top of the environment-side of the browser window. Log in with the same credentials you just used: username `{user}`, password `{password}`:
+
...
. In the **Your Assistant** chat panel on the right, type the following question (if needed, adjust the center divider to reveal the chat panel):
+
[source,console,role=execute,subs=attributes+]
----
