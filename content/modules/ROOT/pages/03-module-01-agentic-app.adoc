...
----
oc login --insecure-skip-tls-verify {openshift_api_url} -u {user} -p {password}
----
...
----
+
Alternatively, you can find the route in the OpenShift console. Navigate to **Networking > Routes** in the `wksp-{user}` project and filter by Name `mortgage-ai-ui`:
+
