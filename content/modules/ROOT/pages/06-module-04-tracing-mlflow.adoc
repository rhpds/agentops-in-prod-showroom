...
. Access the MLflow UI directly from the *MLflow Console* tab at the top of this page:
+
image::agentic-app-pre8.png[Showroom tabs showing MLflow Console tab,link=self,window=blank,width=700]
+
Alternatively, in the OpenShift Console, click the **application launcher** (waffle icon) in the top-right corner. Under **OpenShift Self Managed Services**, click **MLflow**:
+
...
+
Authentication is handled via OpenShift OAuth. Log in with your OpenShift credentials: username `{user}`, password `{password}`.

. Select your workspace from the **Select workspace** dropdown in the top-left corner. Choose `{user_project}`. Your workspace contains the traces and experiments for your mortgage-ai app.
+
...
NOTE: RHOAI {rhoai_version} ships with MLflow {mlflow_version}, but may be updated in future releases.
